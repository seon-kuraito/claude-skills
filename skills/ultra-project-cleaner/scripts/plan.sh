#!/usr/bin/env bash
#
# plan.sh — read-only preview for ultra-project-cleaner. Finds the state Claude
...
jq -n --arg created "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg mode "$mode" --arg only "${only:-all}" \
  ...
  '{tool: "ultra-project-cleaner", version: 1, created: $created, mode: $mode, only: $only,
    targets: $targets, env: $env, scanned: $scanned, items: $items, info: $info}' > "$out"

# ---------------------------------------------------------------- report
echo "== ultra-project-cleaner plan — $mode${only:+ ($only only)}"
jq -r '
