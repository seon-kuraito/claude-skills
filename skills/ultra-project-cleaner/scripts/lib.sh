#!/usr/bin/env bash
#
# lib.sh — shared locations and helpers for plan.sh and apply.sh. Sourced, not run.
#
# Every location can be overridden through the environment, so both scripts can
...
WS_DIR="$VSCODE_USER_DIR/workspaceStorage"
STORAGE_JSON="$VSCODE_USER_DIR/globalStorage/storage.json"
...
require_env() {
  ...
  local t
  for t in jq sqlite3 perl lsof ditto; do
...
# Absolute form of a path that may no longer exist: expands ~, resolves a
# relative path against $PWD, drops trailing slashes. Does not touch the disk.
abspath() {
  local p="$1"
...
# file:///a%20b -> /a b; prints nothing for a non-file URI.
uri_to_path() {
  case "$1" in
    file://*) printf '%s' "${1#file://}" | perl -pe 's/%([0-9A-Fa-f]{2})/chr(hex($1))/eg' ;;
  esac
}
