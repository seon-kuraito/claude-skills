...
使用 [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) 的格式撰寫 git commit 訊息，並要求 description 是單句、祈使語氣的英文。
...
替 git commit 撰寫訊息，格式固定為 Conventional Commits 的 `<type>(<scope>): <description>`（例如：`feat(00-blank): add intro fade-in animation`）。type 與 `ultra-branch-creator` 共用同一套詞彙。詳細規格見 `SKILL.md`。
...
- **與 ultra-branch-creator 共用同一套 type 語彙**：
...
- **預設不寫 prose body**：
  ...
  - 延伸脈絡應留給 PR 說明，不放進 commit body；但也不為了避開 body，硬把一個完整改動拆得過碎
- **依序判定 scope**：
...
- **委派的 skill**：
  - branch 命名交給 `ultra-branch-creator`；沒有該 skill 時，branch 改依 `<type>/<kebab-description>` 慣例自行命名即可
