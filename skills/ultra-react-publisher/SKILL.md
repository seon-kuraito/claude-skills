...
1. **Scaffold (repo-creator · Vite + React template).** Run repo-creator's framework template, choosing **Vite + React** — it owns the scaffold command, the version pin, the `git init`, and the initial commit. The app rides in on the first push to `main`.
