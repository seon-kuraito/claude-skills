#!/usr/bin/env bash
#
# plan.sh — read-only preview for ultra-project-migrator. Checks each move,
# finds the Claude Code state tied to the old paths, lists hard-coded old paths
# inside the projects, asks ultra-project-cleaner (when installed) to plan the
...
if [ -n "$cdir" ] && [ -f "$cdir/scripts/plan.sh" ]; then
  ...
  if bash "$cdir/scripts/plan.sh" --paths "${olds[@]}" --only vscode --no-live-guard --out "$cm" > "$mdir/cleaner-plan.txt" 2>&1; then
    ...
    warning "ultra-project-cleaner could not plan the old paths' VS Code state (see cleaner-plan.txt)"
    cleaner="$(jq -nc --arg d "$cdir" '{installed: true, dir: $d, manifest: null, items: 0}')"
  fi
else
  warning "ultra-project-cleaner is not installed — the old paths' VS Code state will stay behind"
fi
...
jq -n --arg created "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --argjson env "$(env_json)" \
  ...
  '{tool: "ultra-project-migrator", version: 1, created: $created, env: $env, pairs: $pairs,
...
jq -r '
  ...
  "\nVS Code state of the old paths: " + (if .cleaner.manifest then "\(.cleaner.items) item(s) via ultra-project-cleaner" elif .cleaner.installed then "not planned" else "ultra-project-cleaner not installed" end),
