...
A hook is **a command registered in `settings.json` that fires deterministically when a lifecycle event occurs** — not a package the model chooses to load. This is the load-bearing distinction from a skill (see [sk-skill-author](../sk-skill-author/SKILL.md)): a skill is selected by the model from its `description`; a hook always runs when its event fires and its matcher matches. So there is no "trigger description" to tune — correctness lives in the event choice, the matcher / `if` filter, and the script's input / output contract.

## Naming convention

All hooks this user authors — global or project-specific — follow the pattern **`sk-<single-token>-<verber>`**, the same convention as skills. Example: `sk-task-notifier`.
...
- [`../sk-skill-author/references/verification.md`](../sk-skill-author/references/verification.md) — the family verification contract: tiers, runner interface, shared rules
...
- [sk-skill-author](../sk-skill-author/SKILL.md) — the sibling for **skills**, sharing the interview → draft → review shape. Hooks are not skills: deterministic event scripts, not LLM-triggered packages.
