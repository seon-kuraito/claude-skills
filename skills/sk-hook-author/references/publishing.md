...
2. **Open a branch** — hand off to [sk-branch-creator](../../sk-branch-creator/SKILL.md). The type follows the change: `feat/add-<hook-name>` for a new hook; `fix` / `refactor` / `docs` for editing one.
...
5. **Link** (new hook only) — run the repo's `scripts/link-hook.sh <hook-name>` to symlink it into `~/.claude/hooks/`.
...
9. **Commit** — hand off to [sk-commit-creator](../../sk-commit-creator/SKILL.md).
