...
- **A log goes to `~/.claude/logs/<hook-name>.log`.** Every hook that keeps a log writes there, one file per hook, named after the hook. Never inside the hook's own directory: that directory is the repo's working tree behind a symlink, a log line holds real machine paths, and the `no-real-paths` rule scans every file it finds there. Append one line per event, keep the fields fixed, and cut anything taken from tool input to a short length with control characters removed. The log is for the user to read afterwards — a decision must never depend on it, so a log that cannot be written changes nothing (`… >> "$log" 2>/dev/null || true`). Give the path an override for tests, or point `HOME` at a temp directory, so a test run never writes into the real log.
