...
- **`# H1` + tagline + `## 聲明` + the three frame sections** — always present. The H1 is the hook name **without the `sk-` prefix**, in **Title Case** (`Task Notifier`) — same acronym / filename rules as skills (`PR`, `CLAUDE.md`). The tagline is a one-line, keyword-bearing gist of what the hook does and on which event.
- **The three frame headings are fixed, verbatim** — `## 設計背景（WHY）` / `## 功能範圍（WHAT）` / `## 使用方式（HOW）` (the skill READMEs carry the same three headings). The trailing `（WHY/WHAT/HOW）` is a deliberate exception to the no-parenthetical / no-English heading rule (see Tone & language).
...
- **`## 設計背景（WHY）`** — the pain points / motivation this hook addresses, as `- **label**：` bullets framed as the problem (the design choices that answer them live in `### 設計取向`).
- **`## 功能範圍（WHAT）`** — what the hook does and when it fires: the event, the gate, the side-effect, as bullets more specific than the tagline. The README is the hook's main doc, so this can run a touch fuller than a skill's. A hook has no standalone spec file, so close WHAT with the analog of a skill's `SKILL.md` pointer — **主要實作集中在 hook.sh** → [`hook.sh`](hook.sh) (its `install.sh` and any app source are covered under HOW, not here). That label fits a hook whose logic sits in `hook.sh`. When the implementation spans several files — a loader plus one file per rule group, say — the label would state something false, so it may name the structure instead; the bullet then links each file, `hook.sh` first. Only the three frame headings are fixed word for word.
- **`## 使用方式（HOW）`** — a wrapper holding these `h3` subsections:
