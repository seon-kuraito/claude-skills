...
- **hook 建立環節繁多**：
  - 建立 hook 涉及事件選擇、腳本、註冊、測試與安全性檢查；缺少固定流程時容易出錯或遺漏
- **註冊設定容易遺漏或出錯**：
...
- **適用範圍包含所有自建 hook**：
  - 全域 hook 統一放在 `claude-hooks` repo，再將整包目錄逐一 symlink 到 `~/.claude/hooks/`；只服務單一專案的 hook 則作為例外，直接放進該專案版控，並登記在專案的 `.claude/settings.json`
  - 直接透過第三方安裝的 hook 不納入這套流程
