#!/usr/bin/env bash
#
# apply.sh — execute an sk-project-migrator manifest. Run it from Terminal.app
...
# history.jsonl, then runs sk-project-cleaner for the old paths' VS Code
...
# ---------------------------------------------------------------- preflight
echo "== sk-project-migrator apply"
...
if [ -n "$cm" ] && [ -f "$cm" ] && [ -f "$cdir/scripts/apply.sh" ]; then
  echo "  handing the old paths' VS Code state to sk-project-cleaner"
  if ! bash "$cdir/scripts/apply.sh" "$cm" | sed 's/^/    /'; then
    echo "  warning: sk-project-cleaner did not finish; the old paths' VS Code state may remain"
  fi
else
