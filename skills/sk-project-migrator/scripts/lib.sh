...
#   CLEANER_DIR      (default: sk-project-cleaner resolved from its install)
...
# sk-project-cleaner's directory, resolved from its install; empty when absent.
cleaner_dir() {
  if [ -n "${CLEANER_DIR:-}" ]; then
    (cd "$CLEANER_DIR" 2>/dev/null && pwd -P) || true
  elif [ -e "$CLAUDE_DIR/skills/sk-project-cleaner" ]; then
    (cd "$CLAUDE_DIR/skills/sk-project-cleaner" 2>/dev/null && pwd -P) || true
  fi
}
