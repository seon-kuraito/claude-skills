...
**Warnings** — a Claude Code session or another process works inside an old path (it must close before `apply.sh`); `sk-project-cleaner` is not installed or could not plan.
