...
The PR title is the branch name, verbatim — e.g. branch `chore/initial-project-setup` → title `chore/initial-project-setup`. Do not compose an imperative sentence for the title; branch names follow [sk-branch-creator](../sk-branch-creator/SKILL.md), so title quality is already enforced upstream.
