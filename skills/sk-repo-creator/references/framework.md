...
3. **Initial commit.** `git init -b main`, then commit the whole scaffold — **including `package-lock.json`** — as one commit, fixed message `chore: scaffold vite react app` (verbatim — not via sk-commit-creator). `npm create vite` does not init git, so this `git init` is load-bearing.
