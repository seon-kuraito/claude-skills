...
The **Meta Repo** template (from `sk-repo-creator`'s *Pick a template* menu) roots a **new family**: a **coordination layer** plus the member repos listed for it, all created new, side by side in one owner directory. The layer **points at** the members (reached as `../<member>`) and holds shared norms; it never holds their files.
...
- bring existing projects into a family — that is a migration, not a create. Move a folder with `sk-project-migrator`, then add it to the member tables and the workspace by hand;
