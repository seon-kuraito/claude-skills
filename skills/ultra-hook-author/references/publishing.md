...
4. **Link** (new hook only) — run `scripts/link-hook.sh <hook-name>` to symlink it into `~/.claude/hooks/`.
5. **Register** — declare the registration in `settings.hooks.json`, then apply it to the live `~/.claude/settings.json` (see `references/registration.md`). This step is unique to hooks — skills don't have it. Applying to the live file touches the user's runtime config: show the merge, confirm before writing, and consider handing the edit to the `update-config` skill.
6. **Verify** — `/hooks` shows it under the right event, and a fixture run behaves (see `references/testing.md`).
