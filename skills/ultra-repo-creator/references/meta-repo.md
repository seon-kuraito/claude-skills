...
The **Meta Repo** template (from `ultra-repo-creator`'s *Template dispatch*) scaffolds a **coordination layer** over several independent `<prefix>-*` sibling projects — the pattern behind `claude-meta` (over `claude-*`), `bootcamp-rocket-meta` (over the cohorts), and `personal-meta` (over the personal projects). The layer **points at** the siblings (symlinks) and holds shared norms; it never holds their files.
...
| `.gitignore` | `gitignore` | + one `/{{PREFIX}}-<member>` line per member |
| `LICENSE` | `LICENSE` | verbatim (MIT) |
| `scripts/link-siblings.sh` | `link-siblings.sh` | verbatim (generic `${self%-meta}`) |
...
- `{{MEMBER_IGNORE_LINES}}` — one `/{{PREFIX}}-<token>` line per member (`.gitignore`).
...
- **Do not run `scripts/link-siblings.sh`** — members may not exist or be renamed yet. It is scaffolded for later use.
...
- **`chore/initial-project-setup` branch** — one commit per file, in this order:
  `chore: add .gitignore` → `chore: add README.md` → `chore: add CLAUDE.md` → `chore: add LICENSE` → `chore: add link-siblings.sh`.
...
- run `link-siblings.sh` — run it once members exist under their `<prefix>-` names;
- write any memory cards. The generated `CLAUDE.md` *points at* the operating conventions (author a per-member memory card when a member joins; reach siblings through the in-tree `./<prefix>-*` symlinks) but the template does not execute them.
