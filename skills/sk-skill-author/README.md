...
- **固定執行驗證**：
  - 驗證分為結構、腳本、模型三層；結構與腳本層不消耗模型 token，模型層會為每個案例啟動一個使用 Opus 模型的通用 sub-agent
  - 新建與後續修改皆納入驗證流程
- **完整規格集中在 SKILL.md**：
...
- **驗證成本需可估算**：
  ...
  - 每個案例約消耗 4 萬至 15 萬個 token，實際用量取決於流程需要讀取的檔案量
  - 模型層不使用 Explore 或 Plan 類型的 sub-agent，因為這兩種類型不會載入 CLAUDE.md
  - 待所有案例執行完畢後統一修正，再重新執行驗證，減少不必要的重跑
...
- **審閱階段聚焦於決策**：
  - 審閱時只呈現關鍵結構、取捨與待確認事項
  - 審閱時會完整列出草擬的 `description`，供使用者在寫入檔案前確認觸發條件是否合適
